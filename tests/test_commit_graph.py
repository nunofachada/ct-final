...
from committracker.plugins.commit_graph import display_commit_graph, extract_commit_dates
...
def test_display_commit_graph_error(git_repo_mock):
    ...
    repo_path = 'dummy/path/to/repo'
    with patch('committracker.plugins.commit_graph.extract_commit_dates', return_value={"error": "Test error"}):
        with pytest.raises(PreventUpdate):  # Expect PreventUpdate to halt component update
